...
Performance and maintainability guidance for React single-page applications, designed for AI agents and LLMs. The rules emphasize client-side async patterns, bundle reduction, rendering efficiency, state management, and browser performance techniques that apply across React stacks.
...
3. [Client-Side Data Fetching](#3-client-side-data-fetching) — **MEDIUM-HIGH**
   ...
   - 3.2 [Share Request Deduplication Across Components](#32-share-request-deduplication-across-components)
...
Popular icon and component libraries can have **up to 10,000 re-exports** in their entry file. For many React packages, **it takes 200-800ms just to import them**, affecting both development speed and startup time.

**Why tree-shaking doesn't always help:** Some toolchains still need to traverse the full module graph before they can remove unused exports, so wide barrel files can slow local dev and builds even if final bundles are trimmed.
...
Some bundlers offer import-rewrite plugins that automate this optimization, but direct imports remain the most portable default.

Direct imports provide 15-70% faster dev boot, 28% faster builds, faster startup, and significantly faster HMR.
...
Only pay the download and parse cost when the user actually enables the feature.

### 2.3 Defer Non-Critical Third-Party Libraries

**Impact: MEDIUM (loads after first paint or interaction)**
...
Preload when you have a strong signal that the user is likely to need the feature soon, but avoid preloading every optional path at startup.
...
Automatic deduplication and efficient client-side data access patterns reduce redundant work and network requests.
...
### 3.2 Share Request Deduplication Across Components
...
Optimizing browser rendering reduces layout, paint, and commit work.
